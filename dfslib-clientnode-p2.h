...
public:
    ...
    //
    //loading from mount status
    bool loadedFromMountStatus;
    
//    std::unordered_map<std::string, FileAttr> fileTable;
    dfs_service::FileMap fileTable;
    
    //table mutex
     std::mutex writeAccessMutex;
 //rpc function mutex to synchronize callback and inotify
    std::mutex functionMutex;

    
    //function mutexes
    std::mutex ExistsinServerMutex;
    std::mutex RequestWriteLockMutex;
    ...
    bool ExistsInServer(const string &filename);
    
    bool isServerDelete(const string& filename);
